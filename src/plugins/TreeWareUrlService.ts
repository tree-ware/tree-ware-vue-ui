...
export function TreeWareUrlServicePlugin<P extends TreeWareUrlQueryConfig>(Vue: typeof _Vue, options: TreeWareUrlServiceConfig<P>) {
    Vue.prototype.$treeWareUrlService = new TreeWareUrlService(options.router, options.queryParameters)
}
...
export type TreeWareUrlQuerySpec<P> = { [name in keyof P]?: QueryParameterValue | undefined }
...
export class TreeWareUrlService<P extends TreeWareUrlQueryConfig> {
    ...
    updateState() {
        ...
        const [inUrl, notInUrl] = partition(
            this.definedNames,
            item => urlQueryNames.includes(item)
        )

        inUrl.forEach(name => {
            this.query[name].set(urlQuery[name])
        })
        if (urlQuery[OTHER] === TreeWareUrlOtherQueryParameters.reset) notInUrl.forEach(name => {
    ...
    private getQuery(
        querySpec: TreeWareUrlQuerySpec<P>, other: TreeWareUrlOtherQueryParameters
        ...
        const query = Object.fromEntries(
            Object.entries(querySpec).map(([name, spec]) =>
                [name, spec !== undefined ? spec : this.query[name].get()])
        )
